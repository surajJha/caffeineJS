...
describe("efficiency validation (CAFF-040)", () => {
  ...
  function trace(
    ...
    it(`${name} — caffeine hit-rate ≥ lru-cache (${tolerance >= 0 ? "minus" : "plus"} ${Math.abs(tolerance)})`, () => {
      const cache = caffeine<number, number>({ maximumSize: CAPACITY })
        .recordStats()
        .build();
      ...
      const lRate =
        lruHits + lruMisses === 0 ? 0 : lruHits / (lruHits + lruMisses);
      expect(
        cRate,
        `caffeine=${cRate.toFixed(3)} lru=${lRate.toFixed(3)}`,
      ).toBeGreaterThanOrEqual(lRate - tolerance);
    });
  }
  ...
  trace(
    "loop scan (scan resistance)",
    loopStream(50_000, CAPACITY * 5),
    0.0,
  );
  trace(
    "one-hit-wonder (doorkeeper)",
    oneHitStream(50_000, 50),
    0.0,
  );
